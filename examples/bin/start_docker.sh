...
EOF
  ...
  *"query"*)
    ...
    sed -i "s/<GROUP_BY_MAX_MERGING_DICTIONARY_SIZE>/${TELETRAAN_GROUP_BY_MAX_MERGING_DICTIONARY_SIZE:-100000000}/" $DRUID_CONF_DIR/broker/runtime.properties
    ...
    if [ ! -z "${TELETRAAN_DATASOURCE_MULTI_COMPLEMENT_ALLOWED_NAMESPACE_MAP}" ]; then
      if grep -q "druid.broker.dataSourceMultiComplement.allowedNamespaces" $DRUID_CONF_DIR/broker/runtime.properties; then
        sed -i "s/^druid.broker.dataSourceMultiComplement.allowedNamespaces=.*$/druid.broker.dataSourceMultiComplement.allowedNamespaces=${TELETRAAN_DATASOURCE_MULTI_COMPLEMENT_ALLOWED_NAMESPACE_MAP}/" $DRUID_CONF_DIR/broker/runtime.properties
      else
        sed -i "\$adruid.broker.dataSourceMultiComplement.allowedNamespaces=${TELETRAAN_DATASOURCE_MULTI_COMPLEMENT_ALLOWED_NAMESPACE_MAP}" $DRUID_CONF_DIR/broker/runtime.properties
      fi
    fi
...
EOF
  ...
  *"data"*)
    ...
    sed -i "s/<GROUP_BY_MAX_MERGING_DICTIONARY_SIZE>/${TELETRAAN_GROUP_BY_MAX_MERGING_DICTIONARY_SIZE:-100000000}/" $DRUID_CONF_DIR/historical/runtime.properties
    ...
    # Logging
    sed -i "s/<DRUID_REQUEST_LOGGING_TYPE>/${TELETRAAN_DRUID_REQUEST_LOGGING_TYPE:-noop}/" $DRUID_CONF_DIR/historical/runtime.properties
    sed -i "s/<DRUID_REQUEST_LOGGING_THRESHOLD_MS>/${TELETRAAN_DRUID_REQUEST_LOGGING_THRESHOLD_MS:-0}/" $DRUID_CONF_DIR/historical/runtime.properties
...
EOF
  ...
  *"middleManager"*)
    ...
    sed -i "s/<GROUP_BY_MAX_MERGING_DICTIONARY_SIZE>/${TELETRAAN_GROUP_BY_MAX_MERGING_DICTIONARY_SIZE:-100000000}/" $DRUID_CONF_DIR/middleManager/runtime.properties
