 ...
 */
public class LongsSketchBuildBufferAggregator implements BufferAggregator
{
  ...
  public LongsSketchBuildBufferAggregator(
  ...
  {
    final WritableMemory wmem = getMemory(buf).writableRegion(position, size);
    putSketchIntoCache(sketchCache, buf, position, new LongsSketchWrap(maxMapSize, wmem));
  }
  ...
  public void aggregate(final ByteBuffer buf, final int position)
  {
    ...
    lock.lock();
    try {
      final LongsSketchWrap sketch = sketchCache.get(buf).get(position);
      LongsSketchBuildAggregator.updateSketch(sketch, value);
    }
    finally {
      lock.unlock();
    }
  ...
  public void inspectRuntimeShape(RuntimeShapeInspector inspector)
  {
    ...
    // LongsSketch.update() that is called from @CalledFromHotLoop-annotated aggregate()
    // depending on some variable derived from maxMapSize
    inspector.visit("maxMapSize", maxMapSize);
  }

  static void putSketchIntoCache(final IdentityHashMap<ByteBuffer, Int2ObjectMap<LongsSketchWrap>> sketchCache, final ByteBuffer buf, final int position, final LongsSketchWrap sketch)
  {
    final Int2ObjectMap<LongsSketchWrap> map = sketchCache.computeIfAbsent(buf, b -> new Int2ObjectOpenHashMap<>());
