 ...
 * The reason not able to take external jar as dependency is that this is a package level
...
class ReversePurgeLongHashMap
{
  ...
  {
    lgLength = com.yahoo.sketches.Util.toLog2(mapSize, "mapSize");
  ...
  String serializeToString()
  {
    ...
    sb.append(StringUtils.format("%d,%d,", numActive, keys.length));

    for (int i = 0; i < keys.length; i++) {
      if (states[i] != 0) {
        sb.append(StringUtils.format("%d,%d,", keys[i], values[i]));
      }
    }
    return sb.toString();
  }
  ...
  public String toString()
  {
    ...
    for (int i = 0; i < keys.length; i++) {
      ...
      sb.append(StringUtils.format(fmt, i, states[i], values[i], keys[i])).append(com.yahoo.sketches.Util.LS);
    }
    return sb.toString();
  }
