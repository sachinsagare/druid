...
/**
 * This is used for generating test data for {@link LongsSketchAggregatorTest}
 */
class GenerateTestData
{
  ...
  private static void generateSketches() throws Exception
  {
    ...
    Path rawPath = FileSystems.getDefault().getPath("frequencies_longs_raw.tsv");
    Path sketchPath = FileSystems.getDefault().getPath("frequencies_longs_sketches.tsv");
    try (BufferedWriter out1 = Files.newBufferedWriter(rawPath, StandardCharsets.UTF_8)) {
      try (BufferedWriter out2 = Files.newBufferedWriter(sketchPath, StandardCharsets.UTF_8)) {
        ...
        for (int i = 0; i < 20000; i++) {
          ...
          long dim1 = (int) Math.round(rand.nextGaussian() * 100);
          String dimension = Long.toString(dim1);
  ...
  private static void writeSketchRecord(BufferedWriter out, String date, String dimension, LongsSketch sketch) throws Exception
  {
    out.write(date);
    out.write("\t");
    out.write(dimension);
    out.write("\t");
    out.write(StringUtils.encodeBase64String(sketch.toByteArray()));
    out.newLine();
  }
}
