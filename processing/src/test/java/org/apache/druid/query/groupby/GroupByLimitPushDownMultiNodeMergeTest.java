...
public class GroupByLimitPushDownMultiNodeMergeTest
{
  ...
  public void setup() throws Exception
  {
    ...
    // E and F have non overlapping values in metA to create distinct groups in these two segments
  ...
  {
    // The two testing segments have non overlapping groups, so the result should be 100% accurate even force limit push
    // down is applied
    ...
    List<ResultRow> resultsWithLimitPushDown = testForcePushLimitDownAccuracyWhenSortHasNonGroupingFieldsHelper(ImmutableMap.of(
        ...
        GroupByQueryConfig.CTX_KEY_IGNORE_FORCE_LIMIT_PUSH_DOWN_VALIDATION, true
    ));

    Assert.assertEquals("[ResultRow{row=[mango, ripe, 16]}, ResultRow{row=[kiwi, raw, 15]}, ResultRow{row=[watermelon, ripe, 14]}, ResultRow{row=[avocado, ripe, 12]}, ResultRow{row=[orange, raw, 11]}]", resultsWithoutLimitPushDown.toString());
    Assert.assertEquals(resultsWithoutLimitPushDown, resultsWithLimitPushDown);
  }
